...
// Local cn helper to merge className values without relying on unavailable '/utils'.
        ...
        <div className="grid gap-1.5">
          {payload.map((item, index) => {
            ...
            const indicatorColor =
              color || (item.payload as any)?.fill || (item as any).color;

            return (
              <div
                key={(item.dataKey as string) ?? `pt-${index}`}
                className={cn(
...
function getPayloadConfigFromPayload(
  ...
  const payloadPayload =
    p.payload && typeof p.payload === "object" && p.payload !== null
      ? (p.payload as Record<string, any>)
      : undefined;
  ...
  return (configLabelKey in config
    ...
    : (config as any)[key]) as ChartConfig[string] | undefined;
}
